...
const EnrollModal: React.FC<EnrollModalProps> = React.memo(({
  ...
  const validateForm = useCallback(() => {
    ...
    if (!formData.email) newErrors.email = t("enrollModal.errors.emailRequired");
    if (!/^\S+@\S+\.\S+$/.test(formData.email)) newErrors.email = t("enrollModal.errors.invalidEmail");
    if (!formData.phone) newErrors.phone = t("enrollModal.errors.phoneRequired");
    setErrors(newErrors);
        ...
        >
          ...
          <form onSubmit={handleSubmit}>
            <div className={styles.formSection}>
              ...
              <div className={styles.coursePreview}>
                ...
                <div className={styles.courseDetails}>
                  <p>{t("enrollModal.duration")}: {course.duration}</p>
                  <p>{t("enrollModal.level")}: {course.level}</p>
                  <p>{t("enrollModal.language")}: {course.language}</p>
                  <p>{t("enrollModal.format")}: {course.courseFormat}</p>
                  <p>{t("enrollModal.certification")}: {course.certificationOffered ? t("enrollModal.yes") : t("enrollModal.no")}</p>
                </div>
              </div>
            </div>

            <div className={styles.formSection}>
              <h3 className={styles.sectionTitle}>{t("enrollModal.personalInfo")}</h3>
              ...
              <div className={styles.formGroup}>
                <label htmlFor="promoCode">{t("enrollModal.promoCode")}</label>
                <input
                  type="text"
                  id="promoCode"
                  name="promoCode"
            ...
            <div className={styles.actionButtons}>
              ...
              >
                {isSubmitting ? t("enrollModal.processing") : t("enrollModal.enrollNow")}
              </button>
              <button
                type="button"
          ...
          <div className={styles.trustSignals}>
            ...
            <p>{t("enrollModal.satisfiedStudents", { count: course.users.length })}</p>
          </div>
